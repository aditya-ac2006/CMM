...
%   CMM_GIT\pipeline
%
% All generated artifacts are kept inside:
%   CMM_GIT\outputs
%
% No absolute path to the old CMM working directory is used.
...
% Locate the frozen PRE-MF condition database inside the repository outputs.
% Prefer the canonical v1.3 location, then fall back to a recursive search.
inputCandidates = { ...
    fullfile(OUTPUT_ROOT,'_PRE_MF_MATRIX_v1_3','TTC','TTC_CONDITION_ASSIGNED_DATABASE.csv'), ...
    fullfile(OUTPUT_ROOT,'_PRE_MF_MATRIX_v1_3','TTC_CONDITION_ASSIGNED_DATABASE.csv') ...
    };

INPUT_CSV = '';
for k = 1:numel(inputCandidates)
    if isfile(inputCandidates{k})
        INPUT_CSV = inputCandidates{k};
        break;
    end
end

if isempty(INPUT_CSV)
    hits = dir(fullfile(OUTPUT_ROOT,'**','TTC_CONDITION_ASSIGNED_DATABASE.csv'));
    if isempty(hits)
        error(['Frozen PRE-MF condition database not found inside repository outputs.\n' ...
               'Expected: %s\n' ...
               'Run the PRE-MF database/characterization stage first.'], ...
               inputCandidates{1});
    end
    INPUT_CSV = fullfile(hits(1).folder,hits(1).name);
end

OUTDIR = fullfile(OUTPUT_ROOT,'_MF_LATERAL_GLOBAL_v1_5_1');
if ~exist(OUTDIR,'dir'), mkdir(OUTDIR); end

fprintf('[REPOSITORY]\n');
fprintf('Repository root : %s\n',REPO_ROOT);
fprintf('Output root     : %s\n',OUTPUT_ROOT);
fprintf('Input database  : %s\n',INPUT_CSV);
fprintf('MF output       : %s\n\n',OUTDIR);
