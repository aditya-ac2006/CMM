function Result = CMM_LATERAL_MODEL_LOCK_v1_4()
% CMM_LATERAL_MODEL_LOCK_v1_4
% ================================================================
...
% OUTPUT
%   CMM_LATERAL_MODEL_LOCK_v1_4/
%       CMM_LATERAL_MODEL_LOCK_v1_4.mat
%       CMM_LATERAL_MODEL_LOCK_REPORT_v1_4.txt
%       MODEL_PARAMETERS_v1_4.csv
%       SWEEP_CHARACTERIZATION_v1_4.csv
%       CONDITION_CHARACTERIZATION_v1_4.csv
%       LOAD_SENSITIVITY_v1_4.csv
%       PRESSURE_SENSITIVITY_v1_4.csv
%       CAMBER_SENSITIVITY_v1_4.csv
%       REPEATABILITY_v1_4.csv
%       SWEEP_SYMMETRY_v1_4.csv
%       MODEL_FIT_METRICS_v1_4.csv
%       MODEL_CONDITION_METRICS_v1_4.csv
%       MODEL_AUDIT_CHECKS_v1_4.csv
...
fprintf('\n============================================================\n');
fprintf(' CMM LATERAL MODEL LOCK v1.4\n');
...
outputFolder = fullfile(baseRoot,'CMM_LATERAL_MODEL_LOCK_v1_4');
...
%   H4: FY- / SA-
%
% v1.4 freezes the established canonical convention:
%   FY- / SA+
% This is the convention already demonstrated to be compatible with the
% condition-assigned audit database. The four-hypothesis calculation below
% remains diagnostic only; it is NOT allowed to change the locked convention.
...
% Diagnostic best convention is retained for traceability only.
[~,bestConventionIdx]=max(canonicalConvention.R2);
canonicalBestConvention=canonicalConvention(bestConventionIdx,:);

% LOCKED CANONICAL CONVENTION: FY- / SA+
canonicalConventionSignFY=-1;
canonicalConventionSignSA=1;
predC=predC_H2;
canonicalMetrics=M2;
...
[checkName,checkPass,checkDetail]=appendCheck(checkName,checkPass,checkDetail,...
    ...
    sprintf('Diagnostic best %s; audit convention is FY- / SA+',canonicalBestConvention.Convention));
[checkName,checkPass,checkDetail]=appendCheck(checkName,checkPass,checkDetail,...
    "CANONICAL_CONVENTION_FROZEN",...
    canonicalConventionSignFY==-1 && canonicalConventionSignSA==1,...
    sprintf('Frozen convention FY- / SA+ | FY x %.0f | SA x %.0f',...
    canonicalConventionSignFY,canonicalConventionSignSA));
[checkName,checkPass,checkDetail]=appendCheck(checkName,checkPass,checkDetail,...
...
writetable(ParameterTable,fullfile(outputFolder,'MODEL_PARAMETERS_v1_4.csv'));
writetable(SweepCharacteristics,fullfile(outputFolder,'SWEEP_CHARACTERIZATION_v1_4.csv'));
writetable(ConditionCharacteristics,fullfile(outputFolder,'CONDITION_CHARACTERIZATION_v1_4.csv'));
writetable(LoadSensitivity,fullfile(outputFolder,'LOAD_SENSITIVITY_v1_4.csv'));
writetable(PressureSensitivity,fullfile(outputFolder,'PRESSURE_SENSITIVITY_v1_4.csv'));
writetable(CamberSensitivity,fullfile(outputFolder,'CAMBER_SENSITIVITY_v1_4.csv'));
writetable(canonicalConvention,fullfile(outputFolder,'CANONICAL_CONVENTION_DIAGNOSTIC_v1_4.csv'));
writetable(Repeatability,fullfile(outputFolder,'REPEATABILITY_v1_4.csv'));
writetable(SweepSymmetry,fullfile(outputFolder,'SWEEP_SYMMETRY_v1_4.csv'));
writetable(ModelFitMetrics,fullfile(outputFolder,'MODEL_FIT_METRICS_v1_4.csv'));
writetable(ConditionModelMetrics,fullfile(outputFolder,'MODEL_CONDITION_METRICS_v1_4.csv'));

AuditChecks = table(checkName,checkPass,checkDetail,...
    'VariableNames',{'Check','Pass','Detail'});
writetable(AuditChecks,fullfile(outputFolder,'MODEL_AUDIT_CHECKS_v1_4.csv'));
...
save(fullfile(outputFolder,'CMM_LATERAL_MODEL_LOCK_v1_4.mat'),...
    'Lock','-v7.3');

%% ------------------------------------------------------------------------
% REPORT
% -------------------------------------------------------------------------
reportPath = fullfile(outputFolder,'CMM_LATERAL_MODEL_LOCK_REPORT_v1_4.txt');
fid = fopen(reportPath,'w');

if fid ~= -1
    fprintf(fid,'CMM LATERAL MODEL LOCK v1.4\n');
    fprintf(fid,'============================================================\n\n');
...
% -------------------------------------------------------------------------
fprintf('\n============================================================\n');
fprintf(' CMM LATERAL MODEL LOCK v1.4 COMPLETE\n');
fprintf(' STATUS : %s\n',passFail(LockPass));
fprintf(' OUTPUT : %s\n',outputFolder);
fprintf('============================================================\n');
