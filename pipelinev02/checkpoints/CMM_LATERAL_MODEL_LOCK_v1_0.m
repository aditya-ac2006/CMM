function Result = CMM_LATERAL_MODEL_LOCK_v1_0()
% CMM_LATERAL_MODEL_LOCK_v1_0
% ================================================================
...
% OUTPUT
%   CMM_LATERAL_MODEL_LOCK_v1_0/
%       CMM_LATERAL_MODEL_LOCK_v1_0.mat
%       CMM_LATERAL_MODEL_LOCK_REPORT_v1_0.txt
%       MODEL_PARAMETERS_v1_0.csv
%       SWEEP_CHARACTERIZATION_v1_0.csv
%       CONDITION_CHARACTERIZATION_v1_0.csv
%       LOAD_SENSITIVITY_v1_0.csv
%       PRESSURE_SENSITIVITY_v1_0.csv
%       CAMBER_SENSITIVITY_v1_0.csv
%       REPEATABILITY_v1_0.csv
%       SWEEP_SYMMETRY_v1_0.csv
%       MODEL_FIT_METRICS_v1_0.csv
%       MODEL_CONDITION_METRICS_v1_0.csv
%       MODEL_AUDIT_CHECKS_v1_0.csv
%       FIGURES/*.png
%
% ================================================================
...
fprintf('\n============================================================\n');
fprintf(' CMM LATERAL MODEL LOCK v1.0\n');
...
% -------------------------------------------------------------------------
CFG.Version = "1.0";
CFG.ModelFamily = "CMM MF LATERAL GLOBAL v1.5.1";
CFG.PrimaryRuns = [2 4];
...
outputFolder = fullfile(baseRoot,'CMM_LATERAL_MODEL_LOCK_v1_0');
...
score = abs(ConditionCharacteristics.Pressure_Mean_psi-P0) + ...
        abs(ConditionCharacteristics.IA_Mean_deg-IA0) + ...
        abs(ConditionCharacteristics.Speed_Mean_mph-25);
...
LoadSensitivity = ConditionCharacteristics(:, ...
    {'ConditionID','FZ_Mean_N','PeakFY_Mean_N','MuY_Mean',...
     'CorneringStiffness_Mean_N_per_deg','PeakSlipAngle_Mean_deg'});

LoadSensitivity.ForcePerLoad = ...
    LoadSensitivity.PeakFY_Mean_N ./ max(LoadSensitivity.FZ_Mean_N,eps);

PressureSensitivity = buildStateSensitivity(ConditionCharacteristics,'PRESSURE');
CamberSensitivity = buildStateSensitivity(ConditionCharacteristics,'CAMBER');
...
[saC,fyC,fzC,iaC,pC] = extractPhysics(Primary,true);

predC = cmmMFglobal(q,saC,fzC,iaC,pC,Fz0,P0);

canonicalMetrics = computeMetrics(fyC,predC);

fprintf('Canonical R2   : %.6f\n',canonicalMetrics.R2);
fprintf('Canonical RMSE : %.3f N\n',canonicalMetrics.RMSE_N);
fprintf('Canonical MAE  : %.3f N\n',canonicalMetrics.MAE_N);
...
if any(refMask)
    measuredRef = abs(fyA(refMask));
    measuredPeakMu = max(measuredRef)/mean(fzA(refMask));
    measuredPeakAlpha = abs(saA(refMask(find(measuredRef==max(measuredRef),1))));
else
    measuredPeakMu = NaN;
...
% 07: measured characterization trends
fig = figure('Color','w','Visible','off');
plot(ConditionCharacteristics.FZ_Mean_N,...
     ConditionCharacteristics.MuY_Mean,'o-','LineWidth',1.3);
xlabel('F_z [N]');
ylabel('\mu_y');
title('Measured Peak Friction vs Load');
grid on;
saveas(fig,fullfile(figureFolder,'07_MEASURED_MU_VS_LOAD.png'));
close(fig); figureCount=figureCount+1;

% 08: measured cornering stiffness
fig = figure('Color','w','Visible','off');
plot(ConditionCharacteristics.FZ_Mean_N,...
...
title('Measured Cornering Stiffness vs Load');
grid on;
...
[checkName,checkPass,checkDetail]=appendCheck(checkName,checkPass,checkDetail,...
    "FIGURES_WRITTEN",figureCount>=8,sprintf('%d engineering figures',figureCount));
...
writetable(ParameterTable,fullfile(outputFolder,'MODEL_PARAMETERS_v1_0.csv'));
writetable(SweepCharacteristics,fullfile(outputFolder,'SWEEP_CHARACTERIZATION_v1_0.csv'));
writetable(ConditionCharacteristics,fullfile(outputFolder,'CONDITION_CHARACTERIZATION_v1_0.csv'));
writetable(LoadSensitivity,fullfile(outputFolder,'LOAD_SENSITIVITY_v1_0.csv'));
writetable(PressureSensitivity,fullfile(outputFolder,'PRESSURE_SENSITIVITY_v1_0.csv'));
writetable(CamberSensitivity,fullfile(outputFolder,'CAMBER_SENSITIVITY_v1_0.csv'));
writetable(Repeatability,fullfile(outputFolder,'REPEATABILITY_v1_0.csv'));
writetable(SweepSymmetry,fullfile(outputFolder,'SWEEP_SYMMETRY_v1_0.csv'));
writetable(ModelFitMetrics,fullfile(outputFolder,'MODEL_FIT_METRICS_v1_0.csv'));
writetable(ConditionModelMetrics,fullfile(outputFolder,'MODEL_CONDITION_METRICS_v1_0.csv'));

AuditChecks = table(checkName,checkPass,checkDetail,...
    'VariableNames',{'Check','Pass','Detail'});
writetable(AuditChecks,fullfile(outputFolder,'MODEL_AUDIT_CHECKS_v1_0.csv'));
...
save(fullfile(outputFolder,'CMM_LATERAL_MODEL_LOCK_v1_0.mat'),...
    'Lock','-v7.3');

%% ------------------------------------------------------------------------
% REPORT
% -------------------------------------------------------------------------
reportPath = fullfile(outputFolder,'CMM_LATERAL_MODEL_LOCK_REPORT_v1_0.txt');
fid = fopen(reportPath,'w');

if fid ~= -1
    fprintf(fid,'CMM LATERAL MODEL LOCK v1.0\n');
    fprintf(fid,'============================================================\n\n');
...
% -------------------------------------------------------------------------
fprintf('\n============================================================\n');
fprintf(' CMM LATERAL MODEL LOCK v1.0 COMPLETE\n');
fprintf(' STATUS : %s\n',passFail(LockPass));
fprintf(' OUTPUT : %s\n',outputFolder);
fprintf('============================================================\n');
